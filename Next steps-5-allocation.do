...
*Antoinette asked me to do this on 19th November 2019. 
{
reg ln_n_speed_s alloc_off $covars [pw=weight], cluster( person_week )
outreg2 using "$regdir\t1.tex", replace  $outreg_setting ctitle(All waves)
...
drop if alloc_off==0
duplicates drop recruitmentid_new startdate, force
keep recruitmentid_new startdate
bysort startdate: gen no_of_peers=_N 
save "C:\Users\sumit\Documents\Work\Worker Sorting\My work\dta\peers.dta", replace 

restore 
cap drop _merge
merge m:1 recruitmentid_new startdate using "C:\Users\sumit\Documents\Work\Worker Sorting\My work\dta\peers.dta"
replace

replace no_of_peers=0 if no_of_peers==.

*****

gen peer_categ=1 if no_of_peers<9
replace peer_categ=2 if no_of_peers<17 & no_of_peers>8
replace peer_categ=3 if no_of_peers>16


*Accounting for peer effect
reg ln_n_speed_s i.alloc_off#i.peer_categ $covars , cluster( person_week )
outreg2 using "$regdir\t1.tex", replace  $outreg_setting ctitle(All waves)

reg ln_n_speed_s i.alloc_off#i.peer_categ $covars  if wave==5 , cluster( person_week )
outreg2 using "$regdir\t1.tex", append  $outreg_setting ctitle(Wave 3.5)

reg ln_n_speed_s i.alloc_off#i.peer_categ $covars  if wave==6, cluster( person_week )
outreg2 using "$regdir\t1.tex", append  $outreg_setting ctitle(Wave 4)

reg ln_n_speed_s i.alloc_off#i.peer_categ $covars  if wave>4 , cluster( person_week )
outreg2 using "$regdir\t1.tex", append  $outreg_setting ctitle(Wave 3.5 \& 4)

reg ln_n_speed_s i.alloc_off#i.peer_categ $covars ln_n_speed_cash, cluster( person_week )
outreg2 using "$regdir\t1.tex", append  $outreg_setting ctitle(All waves)

reg ln_n_speed_s i.alloc_off#i.peer_categ $covars ln_n_speed_cash if wave==5 , cluster( person_week )
outreg2 using "$regdir\t1.tex", append  $outreg_setting ctitle(Wave 3.5)

reg ln_n_speed_s i.alloc_off#i.peer_categ $covars ln_n_speed_cash if wave==6, cluster( person_week )
outreg2 using "$regdir\t1.tex", append  $outreg_setting ctitle(Wave 4)

reg ln_n_speed_s i.alloc_off#i.peer_categ $covars ln_n_speed_cash if wave>4 , cluster( person_week )
outreg2 using "$regdir\t1.tex", append  $outreg_setting ctitle(Wave 3.5 \& 4)
